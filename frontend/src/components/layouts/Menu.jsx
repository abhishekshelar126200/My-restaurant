...
export default function Menu  ({oncartChange,isLogin}) {
  ...
  const [userLogin,setUserLogged]=useState(isLogin);
          ...
          {displayFood!=null ? 
            (Object.keys(displayFood).map(key=>(
                <div>
                  ...
                  <hr/>
                  <div className='row'>
                      <FoodItem foodItem={displayFood[key]} oncartChange={oncartChange} rest={hotel} restId={restId} isLogin={userLogin}/>
                  </div>
                </div>
              ))
            )
